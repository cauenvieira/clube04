(() => {
    ...
    // Parte 1: Verifica se estamos na página correta.
    // Usamos .split('?')[0] para ignorar parâmetros como ?status=ok
    if (window.location.href.split('?')[0] !== paginaAlvo) {
        
        // Se NÃO ESTIVER, armazena um "gatilho" e redireciona.
        console.log("Página incorreta. Preparando para redirecionar...");
        alert("Você não está na página de ponto. Redirecionando agora...\n\nO painel aparecerá automaticamente na próxima página.");
        
        sessionStorage.setItem('iniciarGeradorPonto', 'true'); // O gatilho!
        window.location.href = paginaAlvo;

    } else {
        // Se ESTIVER na página correta, ou se acabamos de ser redirecionados...
        
        // Parte 2: A função que cria o painel e toda a mágica visual.
        // A colocamos aqui para só ser executada no lugar certo.
        function rodarGerador() {
            console.log("Executando gerador de relatório na página correta.");
        ...
        // Parte 3: Decide se o gerador deve ser executado
        // Verificamos se o "gatilho" existe (veio de um redirecionamento) ou se o script foi chamado manualmente na página certa.
        if (sessionStorage.getItem('iniciarGeradorPonto') === 'true') {
            sessionStorage.removeItem('iniciarGeradorPonto'); // Remove o gatilho para não rodar de novo em um refresh
            rodarGerador();
        } else {
            // Se não veio de um redirect, é um clique manual, então apenas rodamos.
            rodarGerador();
        }
    }
})();
